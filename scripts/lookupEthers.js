...
const loadCollectionsData = async () => {
    ...
    if (currentChain == 1) {
        ...
        let collectionsGated = await marketGated.getAllEnabledContracts();
        let numCollectionsGated = collectionsGated.length;
        let allCollectionIdsGated = Array.from(Array(numCollectionsGated).keys());
        await Promise.all(allCollectionIdsGated.map(async (i) => {
            let collectionAddress = collectionsGated[i];
            try {
                if ((await marketGated.isAuthorized(collectionAddress, userAddress))) {
                    $("#workshop-link").removeClass("hidden");
                    $("#mobile-workshop-link").removeClass("hidden");
                }
            }
            ...
            let projectInfo = await marketGated.contractToProjectInfo(collectionAddress);
            let projectName = projectInfo.projectName;
            let listingIdsToInfo = new Map();
            let listingsToBuyers = new Map();
            let numListings = Number(await marketGated.getWLVendingItemsLength(collectionAddress));
            let allListingIds = Array.from(Array(numListings).keys());
            const chunks = splitArrayToChunks(allListingIds, 10);
            for (const chunk of chunks) {
                await Promise.all(chunk.map(async (id) => {
                    let buyers = (await marketGated.getWLPurchasersOf(collectionAddress, id));
                    let WLinfo = await marketGated.contractToWLVendingItems(collectionAddress, id);
                    let title = WLinfo.title;
                    let purchased = buyers.includes(userAddress) ? true : false;
                    if (purchased) {
                        myWL.push(title.toUpperCase());
                    ...
                    listingIdsToInfo.set(id + numCollections, { title: `#${id + numCollections}: ${title}`, discordsAndBuyers: discordsAndBuyers });
                }));
            }
            for (const listingId of allListingIds) {
                let listing = listingIdsToInfo.get(listingId + numCollections);
                listingsToBuyers.set(listing.title, listing.discordsAndBuyers)
            }
            if (!(projectToWL.get(projectName))) {
                projectToWL.set(projectName, listingsToBuyers);
            }
            else {
                let currentListingsToBuyers = projectToWL.get(projectName);
                projectToWL.set(projectName, new Map([...currentListingsToBuyers, ...listingsToBuyers]));
            }
            fakeJSX = `<option value="${projectName}">${projectName.toUpperCase()}</option>`;
            collectionIdToJSX.set(i, fakeJSX);
        }))
