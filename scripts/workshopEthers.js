...
const loadPartnerCollections = async () => {
    ...
    for (let i = 0; i < gatedCollections.length; i++) {
        ...
        let approved;
        try {
            approved = await marketGated.isAuthorized(address, userAddress);
        }
        catch {
            approved = false;
        }
    ...
    try {
        ...
        if (!currentProjectAddress) {
            ...
            if (!(title && image && (site != null) && (description != null) && amount && start && deadline && price)) {
                ...
                if (gatedCollections.includes(currentProjectAddress)) {
                    const gasLimit = await marketGated.estimateGas.addWLVendingItem(currentProjectAddress, [title, image, site, description, amount, 0, start, deadline, price]);
                    const newGasLimit = parseInt((gasLimit * 1.15)).toString();
                    await marketGated.addWLVendingItem(currentProjectAddress, [title, image, site, description, amount, 0, start, deadline, price], { gasLimit: newGasLimit }).then(async (tx_) => {
